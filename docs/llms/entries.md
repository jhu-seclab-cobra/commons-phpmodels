...
- A callable `signature:` fixes the arity: an argument port (guard, propagation side, sink) at a position outside the declared parameter list is rejected, unless the last parameter is variadic. An entry without a signature is not arity-checked.
