...
that translates one set's names into the merge's. Base entities — Model,
...
- **Document Set** — One stored source of models: a root, a manifest, at
  ...
  sets is its Mount Order ([model-index.md](model-index.md)).
- **Set Provenance** — The statement of where a set's entries come from: a
  ...
  merge's default kind stands in. One per set; it applies to every entry
...
- **Precedence** — A total order over verification kinds, highest first.
  Existence condition: supplied to one merge for all the sets it mounts.
...
- **Accumulated Vocabulary** — The vocabulary in force while the merge
  mounts sets in order: the union of every set's declarations mounted so far.
...
- **Category Mapping** — A total function, supplied with one set's load,
  from the names that set uses to the accumulated vocabulary's names, one
  relation per axis: danger category to danger category, origin color to
  origin color. Each source name is mapped to one declared target name or
  marked discarded. Never stored with the set it translates.
...
| Document Set | Policy | declares | 1:0..1 | Rows accumulate into the index's policy |
...
| equal | equal | the later Mount Order wins |
| absent (no provenance document) | any | A ranks as the merge's default kind |
...
- **Mapped ≡ written.** After translation an entry is indistinguishable
  from one written in the accumulated vocabulary's names; no reader of the
  index sees the source names.
